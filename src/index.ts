  ...
  extends (String as any as { new (base: string): { toString: () => string } })
{
  ...
  /*#__PURE__*/
  constructor(...args: Array<string | CSSVariableOptions<TValue>>) {
...
type ICreateVar = {
  ...
  <TValue>(
    uniqueName: string,
    options: CSSVariableOptions<TValue>
  ): CSSVariable<TValue>;
};
export const createVar: ICreateVar = (...args: any[]) =>
  new (CSSVariable as any)(...args);
...
export type CSSVariableValueArgument<T> = T extends CSSVariable<infer U>
  ? U
  : T;
/**
...
type ThemeValues<TThemeStructure extends ThemeStructure> = {
  [Property in keyof TThemeStructure]: TThemeStructure[Property] extends CSSVariable
    ?
        | CSSVariableValueArgument<TThemeStructure[Property]>
        | CSSVariable<CSSVariableValueArgument<TThemeStructure[Property]>>
  ...
  Object.keys(cssVariableValues)
    .map((key) =>
      typeof cssVariableValues[key] === "string"
        ? (cssVariables[key] as CSSVariable).toCSS(
            cssVariableValues[key] as string
          )
        : assignVars(
 ...
 * ```
 */
export const createGlobalTheme = <TTheme extends ThemeStructure>(
  scope: string | undefined | null,
  cssVariables: TTheme,
  cssVariableValues: ThemeValues<TTheme>
): string =>
  `${scope ? `${scope}{` : ""}${assignVars(
    cssVariables,
    cssVariableValues as DeepPartial<ThemeValues<TTheme>>
  )}${scope ? "}" : ""}`;
